    ...
    <div class="section-body">
            ...
            <div class="col-md-10">
                <div class="row mb-0 mt-3">
                    <div class="col-md-6 mb-2">
                        <label for="" class="form-label pl3" style="font-size: 14px;"><b><i class="fa-solid fa-building-user"></i> บริษัท :</b> <?php echo $company;?></label>
                    </div>
                    <div class="col-md-6 mb-2">
                        <label for="" class="form-label pl3" style="font-size: 14px;"><b><i class="fa-solid fa-house-user"></i> สาขา :</b> <?php echo $branch;?></label>
                    </div>
                </div>
                <div class="row mb-0">
                    <div class="col-md-6 mb-2">
                        <label for="" class="form-label pl3" style="font-size: 14px;"><b><i class="fa-solid fa-id-badge"></i> ตำแหน่งที่ต้องการสมัคร :</b> <?php echo $row['job_name'];?></label>
                    </div>
                    <div class="col-md-6 mb-2">
                        <label for="" class="form-label pl3" style="font-size: 14px;"><b><i class="fa-solid fa-sack-dollar"></i> เงินเดือนที่ต้องการ :</b> <?php if (stripos($row['salary'], ',')) {echo $row['salary']." บาท";}else{echo number_format($row['salary'])." บาท";};?></label>
                    </div>
                </div>
                <div class="row mb-0">
                    <div class="col-md-6 mb-2">
                        <label for="" class="form-label pl3" style="font-size: 14px;"><b><i class="fa-solid fa-calendar-plus"></i> วันที่กรอกใบสมัคร :</b> <?php echo $row['date_create'];?></label>
                    </div>
                    <div class="col-md-6 mb-2">
                        <label for="" class="form-label pl3" style="font-size: 14px;"><b><i class="fa-solid fa-calendar-check"></i> วันที่พร้อมเริ่มงาน :</b> <?php echo $row['work_date'];?></label>
                    </div>
                </div>
            </div>
        </div>   
        ...
        <div class ="style_d1 mb-1">
            ...
            <div class="row mb-0">
                ...
                <div class="col-md-4 mb-1">
                    <label for="" class="form-label pl3"><b>ความสัมพันธ์ :</b> <?php if($row['person_referen_relations'] !=""){echo $row['person_referen_relations'];}else{echo "-";}?></label>
                </div>
            </div>
            <div class="plr"><hr class="style-one mt-0"></div>
            <div class="row mb-0 bt_attact">
        ...
        <div class="style_d1 mb-1">
            ...
            <div style="text-align: center;">
                <button type="button" class="btn btn-primary btn-sm mt-3" onclick="window.open('exportjob_pdf.php?guest_no=<?php echo $_GET['guest_no'];?>')"/>
                <i class="fa-solid fa-file-pdf fa-lg"></i> ปริ้นใบสมัคร to PDF</button>

                <button type="button" class="btn btn-success btn-sm mt-3" onclick="location.href='guest_details.php?status=1&guest_no=<?php echo $_GET['guest_no'];?>';">
                <i class="fa-regular fa-circle-check fa-lg"></i> เรียกสัมภาษณ์ </button>

			    <button type="button" class="btn btn-danger btn-sm mt-3" onclick="location.href='guest_details.php?status=0&guest_no=<?php echo $_GET['guest_no'];?>';">
                <i class="fa-regular fa-circle-xmark fa-lg"></i> ไม่เรียกสัมภาษณ์ </button>
            </div>  
        </div>
...
<?php
if (!isset($_GET['status']) || !isset($_GET['guest_no'])) {
}else{    
   $guest_no = $_GET['guest_no'];
   $status = isset($_GET['status']) ? $_GET['status'] : null;

   $strSQL2 = "SELECT * FROM resume_job WHERE guest_no = ?";
   $params2 = array($guest_no);
   ...
   $modalMessage = '';
   if ($numRows2 <= 0) {
       $modalMessage = "ไม่พบข้อมูลใบสมัครงานของ $guest_no ในระบบ !!";
       $msg="<span><i class='fa-solid fa-circle-xmark fa-lg text-danger'></i> ".$modalMessage."</span>"; 
   } else {
       if ($status == "1") {
           $interviews = "เรียกสัมภาษณ์";
       } elseif ($status == "0") {
           $interviews = "ไม่เรียกสัมภาษณ์";
       } else {
           $interviews = null;
       }

       if ($interviews !== null) {
           $sql = "UPDATE resume_job SET interviews = ? WHERE guest_no = ?";
           $updateParams = array($interviews, $guest_no);
           $query = sqlsrv_query($conn, $sql, $updateParams);

           if($interviews == "เรียกสัมภาษณ์"){
            $modalMessage = "อัปเดทเรียกสัมภาษณ์ของผู้สมัคร $guest_no เรียบร้อยแล้ว !!";
            $msg="<span><i class='fa-solid fa-circle-check fa-lg text-success'></i> ".$modalMessage."</span>";
           }else if($interviews == "ไม่เรียกสัมภาษณ์"){
            $modalMessage = "อัปเดทไม่เรียกสัมภาษณ์ของผู้สมัคร $guest_no เรียบร้อยแล้ว !!";
            $msg="<span><i class='fa-solid fa-circle-check fa-lg text-success'></i> ".$modalMessage."</span>";
           }
       }
   }
